...
describe('episodePaths', () => {
  test('produces correct filenames for a basic episode', () => {
    ...
    expect(basename(paths.transcription)).toBe('123.transcript__base.json');
  ...
  test('zero-pads episode numbers to 3 digits', () => {
    ...
    expect(basename(paths.transcription)).toBe('001.transcript__base.json');
  ...
  test('handelizes model name', () => {
    ...
    expect(basename(paths.transcription)).toBe('001.transcript__gpt-4o.json');
  });
