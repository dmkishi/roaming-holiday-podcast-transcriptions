...
async function transcribeFromFeed(): Promise<void> {
  ...
  const episodes = rss.found.map((w) => w.episode);
...
async function buildTranscripts(): Promise<void> {
  ...
  for (const [episodeNumber, segments] of segmentsByEpisode) {
    ...
    const { path, changed } = writeTranscript(episodeNumber, { paragraphGroups });
    printLog.info([
      `#${episodeNumber}: ${changed ? 'Saved' : 'Unchanged'} "${toRelative(path)}"`,
